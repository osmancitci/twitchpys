Funbit.Ets.Telemetry.Dashboard.prototype.initialize = function (skinConfig, utils) {
	...
	// $.getJSON('https://osmancitci.com.tr/ets2/kur.php', function(data) {
		
   // var gelsinparalar = data.trim();
   
	  // parabirimi = data.trim();
...
Funbit.Ets.Telemetry.Dashboard.prototype.filter = function (data, utils) {
    ...
    if (language == 'de') {
		...
		//parabirimi = 8.699996364473636;
        data._gearIndicatorA   = 'Otomatik Şanzıman Dişli Göstergesi';
        data._gearIndicatorM   = 'Manuel Şanzıman Dişli Göstergesi';
        data._speedometerL     = 'Hız Göstergesi (Sol) Takometre (Sağ)';
        data._speedometerR     = 'Takometre(Sol) Hız Göstergesi (Sağ)';
        ...
        data._jobIncomed        = (data.job.income != 0 && data.game.gameName == 'ATS')
                               ? '$ ' + this.numberToThousandsDE(data.job.income) + ',-'
                               : data.job.income != 0
                               ? this.numberToThousandsDE(data.job.income) * + parabirimi + ' ₺'
                               : '0 ₺';
var sayi1 = this.numberToThousandsDE(data.job.income);
var sayi2 = '' + parabirimi + '';
var toplam = sayi1 * sayi2;
var toparla = '' + toplam + '';
var toparlan = toparla.split("", 7).join("");
        data._jobIncome        = (data.job.income != 0 && data.game.gameName == 'ATS')
                               ? '$ ' + this.numberToThousandsDE(data.job.income) + ',-'
                               : data.job.income != 0
							   ? toparlan + ' ₺'
    ...
    data._trailerWear = data.trailer1.attached ? Math.max(data._trailerWearChassis, data._trailerWearWheels) + '%' : '';
